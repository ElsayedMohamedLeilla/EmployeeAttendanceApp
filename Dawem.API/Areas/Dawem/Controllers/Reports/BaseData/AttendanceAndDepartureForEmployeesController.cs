...
namespace Dawem.API.Areas.Dawem.Controllers.Reports.BaseData
{
    ...
    public class AttendanceAndDepartureForEmployeesController : DawemControllerBase
    {
        private readonly IReportGeneratorBL _reportGeneratorBL;
        public AttendanceAndDepartureForEmployeesController(IReportGeneratorBL reportGeneratorBL)
        {
            _reportGeneratorBL = reportGeneratorBL;
        }
        [HttpPost]
        public IActionResult GetAttendanceAndDepartureForEmployeesReport([FromQuery] AttendanceAndDepartureForEmployeesReportCritria param)
        {
            var response = _reportGeneratorBL.GenerateAttendanceAndDepartureForEmployeesReport(param);
            if (response != null && response.IsSuccessStatusCode)
            {
                ...
                switch (param.ExportFormat)
                {
                    case ExportFormat.Pdf:
                        return File(contentStream, "application/pdf", "AttendanceAndDepartureForEmployeesReport.pdf");
