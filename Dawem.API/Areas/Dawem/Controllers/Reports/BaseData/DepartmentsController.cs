...
namespace Dawem.API.Areas.Dawem.Controllers.Reports.BaseData
{
    ...
    public class DepartmentsController : DawemControllerBase
    {
        private readonly IReportGeneratorBL _reportGeneratorBL;
        public DepartmentsController(IReportGeneratorBL reportGeneratorBL)
        {
            _reportGeneratorBL = reportGeneratorBL;
        }
        [HttpPost]
        public IActionResult GetDepartmentsReport([FromQuery] DepartmentsReportCritria param)
        {
            var response = _reportGeneratorBL.GenerateDepartmentsReport(param);
            if (response != null && response.IsSuccessStatusCode)
            {
                ...
                switch (param.ExportFormat)
                {
                    case ExportFormat.Pdf:
                        return File(contentStream, "application/pdf", "DepartmentsReport.pdf");
