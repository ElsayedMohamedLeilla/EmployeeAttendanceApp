...
namespace Dawem.API.Areas.Dawem.Controllers.Reports.BaseData
{
    ...
    public class SanctionsController : DawemControllerBase
    {
        private readonly IReportGeneratorBL _reportGeneratorBL;
        public SanctionsController(IReportGeneratorBL reportGeneratorBL)
        {
            _reportGeneratorBL = reportGeneratorBL;
        }
        [HttpPost]
        public IActionResult GetSanctionsReport([FromQuery] SancationsReportCritria param)
        {
            var response = _reportGeneratorBL.GenerateSanctionsReport(param);
            if (response != null && response.IsSuccessStatusCode)
            {
                ...
                switch (param.ExportFormat)
                {
                    case ExportFormat.Pdf:
                        return File(contentStream, "application/pdf", "SanctionsReport.pdf");
