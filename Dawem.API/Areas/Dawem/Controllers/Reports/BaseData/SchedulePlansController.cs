...
namespace Dawem.API.Areas.Dawem.Controllers.Reports.BaseData
{
    ...
    public class SchedulePlansController : DawemControllerBase
    {
        private readonly IReportGeneratorBL _reportGeneratorBL;
        public SchedulePlansController(IReportGeneratorBL reportGeneratorBL)
        {
            _reportGeneratorBL = reportGeneratorBL;
        }
        [HttpGet]
        public IActionResult GetSchedulePlansReport([FromQuery] SchedulePlansReportCritria param)
        {
            var response = _reportGeneratorBL.GenerateSchedulePlansReport(param);
            if (response != null && response.IsSuccessStatusCode)
            {
                ...
                switch (param.ExportFormat)
                {
                    case ExportFormat.Pdf:
                        return File(contentStream, "application/pdf", "SchedulePlansReport.pdf");
