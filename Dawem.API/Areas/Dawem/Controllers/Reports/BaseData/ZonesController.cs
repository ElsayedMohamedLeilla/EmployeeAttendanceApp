...
namespace Dawem.API.Areas.Dawem.Controllers.Reports.BaseData
{
    ...
    public class ZonesController : DawemControllerBase
    {
        private readonly IReportGeneratorBL _reportGeneratorBL;
        public ZonesController(IReportGeneratorBL reportGeneratorBL)
        {
            _reportGeneratorBL = reportGeneratorBL;
        }
        [HttpPost]
        public IActionResult GetZonesReport([FromQuery] ZonesReportCritria param)
        {
            var response = _reportGeneratorBL.GenerateZonesReport(param);
            if (response != null && response.IsSuccessStatusCode)
            {
                ...
                switch (param.ExportFormat)
                {
                    case ExportFormat.Pdf:
                        return File(contentStream, "application/pdf", "ZonesReport.pdf");
