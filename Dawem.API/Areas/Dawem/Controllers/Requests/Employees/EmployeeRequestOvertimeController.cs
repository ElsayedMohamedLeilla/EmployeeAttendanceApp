...
namespace Dawem.API.Areas.Dawem.Controllers.Requests.DawemAdmins
{
    ...
    public class EmployeeRequestOvertimeController : DawemControllerBase
    {
        private readonly IRequestOvertimeBL requestOvertimeBL;


        public EmployeeRequestOvertimeController(IRequestOvertimeBL _requestOvertimeBL)
        {
        ...
        [HttpGet]
        public async Task<ActionResult> Get([FromQuery] EmployeeGetRequestOvertimeCriteria criteria)
        {
            if (criteria == null)
            {
                return BadRequest();
            }
            var requestOvertimesresponse = await requestOvertimeBL.EmployeeGet(criteria);
