    ...
    public class SchedulePlanRepository : GenericRepository<SchedulePlan>, ISchedulePlanRepository
    {
        ...
        public IQueryable<SchedulePlan> GetAsQueryable(GetSchedulePlansCriteria criteria)
        {
            ...
            if (!string.IsNullOrWhiteSpace(criteria.FreeText))
            {
                ...
                inner = inner.And(x => x.Schedule.Name.ToLower().Trim().StartsWith(criteria.FreeText));
                inner = inner.And(x => x.SchedulePlanEmployee != null && x.SchedulePlanEmployee.Employee.Name.ToLower().Trim().StartsWith(criteria.FreeText));
                inner = inner.And(x => x.SchedulePlanGroup != null && x.SchedulePlanGroup.Group.Name.ToLower().Trim().StartsWith(criteria.FreeText));
                inner = inner.And(x => x.SchedulePlanDepartment != null && x.SchedulePlanDepartment.Department.Name.ToLower().Trim().StartsWith(criteria.FreeText));
